...
void SpecificWorker::initialize_model(const StateVector &target)
{
    ...
    for (uint e = FIRST_NEAR; e <= LAST_NEAR; e++)
    {
        ...
        control_vars[e * CONTROL_DIM].set(GRB_DoubleAttr_LB, -200);
        control_vars[e * CONTROL_DIM].set(GRB_DoubleAttr_UB, +200);

        v_name_v << "v" << e;
        control_vars[e * CONTROL_DIM + 1].set(GRB_StringAttr_VarName, v_name_v.str());
        control_vars[e * CONTROL_DIM + 1].set(GRB_DoubleAttr_LB, -200);
        control_vars[e * CONTROL_DIM + 1].set(GRB_DoubleAttr_UB, +200);
