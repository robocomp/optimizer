...
class ListOfPoints(list):
    def __init__(self, iterable=list()):
        super(ListOfPoints, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompRoomDetection.Corner)
        super(ListOfPoints, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompRoomDetection.Corner)
        super(ListOfPoints, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompRoomDetection.Corner)
        super(ListOfPoints, self).insert(index, item)

setattr(RoboCompRoomDetection, "ListOfPoints", ListOfPoints)
class Rooms(list):
    def __init__(self, iterable=list()):
        super(Rooms, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, RoboCompRoomDetection.ListOfPoints)
        super(Rooms, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, RoboCompRoomDetection.ListOfPoints)
        super(Rooms, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, RoboCompRoomDetection.ListOfPoints)
        super(Rooms, self).insert(index, item)

setattr(RoboCompRoomDetection, "Rooms", Rooms)

import roomdetectionI
...
class Implements:
    def __init__(self, ice_connector, default_handler):
        self.ice_connector = ice_connector
        self.roomdetection = self.create_adapter("RoomDetection", roomdetectionI.RoomDetectionI(default_handler))
